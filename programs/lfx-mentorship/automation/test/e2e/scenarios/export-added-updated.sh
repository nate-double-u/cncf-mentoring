...
BODY_E1=$(pr_body "$E1")
check_eq       "S1 E1 title '(1 new)'"             "chore: LFX export for $LFX_TERM (1 new)" "$(pr_title "$E1")"
...
gh workflow run lfx-export.yml -R "$REPO" -f term="$LFX_TERM" || die "re-dispatch failed"
...
check_eq       "S4 E2 title '(1 new, 1 updated)'"  "chore: LFX export for $LFX_TERM (1 new, 1 updated)" "$(pr_title "$E2")"
...
gh workflow run lfx-export.yml -R "$REPO" -f term="$LFX_TERM" || die "S5 dispatch failed"
...
check_eq       "S5 E3 title '(1 updated)'"           "chore: LFX export for $LFX_TERM (1 updated)" "$(pr_title "$E3")"
